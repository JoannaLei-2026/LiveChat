document.addEventListener('DOMContentLoaded', () => {
  ...
  function initWebSocket(personaPrompt) {
    ...
    ws.onmessage = (event) => {
      try {
        ...
        if (data.type === 'status') {
          updateStatus(data.connected, `Gemini 3.1 Flash Live Preview (${data.mode || 'WebSocket'})`);
          ...
          console.log('Persona initialized successfully');
        }
      } catch (err) {
  ...
  function appendSystemNotice(text) {
    ...
    noticeDiv.style.color = 'var(--accent-cyan)';
